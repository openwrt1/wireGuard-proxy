...
#================================================================================
# 适用于 Debian/Ubuntu 的 WireGuard + Udp2raw 一键安装脚本
#
# 功能:
...
# 判断系统是否为 Debian 或 Ubuntu
check_debian_or_ubuntu() {
	if ! grep -qi -e "Debian" -e "Ubuntu" /etc/os-release; then
		echo "错误: 此脚本仅支持 Debian 或 Ubuntu 系统" 1>&2
		exit 1
	fi
}
...
start_menu() {
	clear
	echo "=================================================="
	echo " 适用于 Debian/Ubuntu 的 WireGuard 一键安装脚本"
