...
wireguard_install() {
    ...
    if ! command -v wg &> /dev/null; then
        echo "错误: 安装 wireguard-tools 失败，无法找到 wg 命令。请检查您的网络和软件源。" >&2
        exit 1
    fi

    # 尝试安装 qrencode
    has_qrencode=true
    if ! apk add qrencode; then
        echo "警告: 安装 qrencode 失败。脚本可以继续，但最后无法生成二维码。"
        has_qrencode=false
    fi
    ...
    nat_rule="-t nat -A POSTROUTING -s 10.0.0.0/24 -o $net_interface -j MASQUERADE"
    postup_rules="iptables -C $nat_rule 2>/dev/null || iptables $nat_rule;"
    predown_rules="iptables -D $nat_rule;"
    ...
    if [ "$use_udp2raw" == "y" ]; then
        ...
        input_rule="-A INPUT -p tcp --dport $tcp_port -j ACCEPT"
        postup_rules="$postup_rules iptables -C $input_rule 2>/dev/null || iptables $input_rule;"
        predown_rules="$predown_rules iptables -D $input_rule;"
        ...
        input_rule="-A INPUT -p udp --dport $wg_port -j ACCEPT"
        postup_rules="$postup_rules iptables -C $input_rule 2>/dev/null || iptables $input_rule;"
        predown_rules="$predown_rules iptables -D $input_rule;"
...
wireguard_uninstall() {
	...
	apk del wireguard-tools qrencode curl iptables
