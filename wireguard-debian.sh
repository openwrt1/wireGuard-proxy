...
wireguard_install() {
    ...
    # 幂等添加 NAT 规则
    if ! grep -q "# WIREGUARD_NAT_RULES" /etc/ufw/before.rules; then
        sed -i '1s;^;# WIREGUARD_NAT_RULES\n*nat\n:POSTROUTING ACCEPT [0:0]\n-A POSTROUTING -s 10.0.0.0/24 -o $net_interface -j MASQUERADE\nCOMMIT\n# END WIREGUARD_NAT_RULES\n\n;' /etc/ufw/before.rules
    fi
...
wireguard_uninstall() {
    ...
    echo "正在清理防火墙 NAT 规则..."
    sed -i '/^# WIREGUARD_NAT_RULES/,/^# END WIREGUARD_NAT_RULES/d' /etc/ufw/before.rules
    ufw reload
