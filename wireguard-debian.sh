...
wireguard_install() {
    ...
    if [ "$USE_UDP2RAW" == "y" ]; then
        ...
        cat > /etc/systemd/system/udp2raw.service <<-EOF
			...
			[Service]
			Type=simple
			ExecStart=/usr/local/bin/udp2raw -s -l 0.0.0.0:$tcp_port -r 127.0.0.1:$wg_port -k "$udp2raw_password" --raw-mode faketcp --cipher-mode xor
			Restart=on-failure
			[Install]
...
optimize_system() {
    ...
    apt-get -t "${DEBIAN_VERSION}-backports" install -y linux-image-amd64
